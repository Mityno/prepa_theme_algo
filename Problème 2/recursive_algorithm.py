...
def aux(end_space, index, word_list, L, mem_dict, line_break_dict):
    # Mémoisation :
    ...
    if len(m) - 1 > end_space:
        mem_dict[(end_space, index)] = (end_space+1)**2 + aux(L - len(m), index+1, word_list, L, mem_dict, line_break_dict)
        line_break_dict[(end_space, index)] = True
        return mem_dict[(end_space, index)]
    

    current_line = aux(end_space - len(m), index+1, word_list, L, mem_dict, line_break_dict)
    new_line = (end_space+1)**2 + aux(L - len(m), index+1, word_list, L, mem_dict, line_break_dict)
...
def least_squares(word_list, L):
    mem_dict = {}
    line_break_dict = {}
    least_error = aux(L, 0, word_list, L, mem_dict, line_break_dict)
    line_list = [[]]
    end_space = L
